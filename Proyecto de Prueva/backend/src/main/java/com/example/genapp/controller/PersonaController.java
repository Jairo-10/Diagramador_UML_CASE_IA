...
import com.example.genapp.service.PersonaService;
...
public class PersonaController {
    ...
    @GetMapping("/{id}")
    public Persona one(@PathVariable("id") String id) {
    return service.findById(id)
    .orElseThrow(() -> new RuntimeException("Persona not found with id " + id));
    }

    @PostMapping
    public Persona create(@RequestBody Map<String, Object> body) {
    ...
    @PutMapping("/{id}")
    public Persona update(@PathVariable("id") String id, @RequestBody Map<String, Object> updates) {
        // Verificar que el recurso existe antes de actualizar
        Persona existing = service.findById(id)
            .orElseThrow(() -> new RuntimeException("Persona not found with id " + id));
    ...
    private void updateEntityFromMap(Persona entity, Map<String, Object> data) {
