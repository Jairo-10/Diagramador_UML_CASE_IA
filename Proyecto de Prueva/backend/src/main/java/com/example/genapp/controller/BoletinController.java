...
import com.example.genapp.service.BoletinService;
...
public class BoletinController {
    ...
    @GetMapping
    public List<Boletin> all() {
    return service.findAll();
    }

    @GetMapping("/{id}")
    public Boletin one(@PathVariable("id") String id) {
    return service.findById(id)
    .orElseThrow(() -> new RuntimeException("Boletin not found with id " + id));
    }

    @PostMapping
    public Boletin create(@RequestBody Map<String, Object> body) {
        Boletin entity = new Boletin();
        updateEntityFromMap(entity, body);
        return service.save(entity);
    }

    @PutMapping("/{id}")
    public Boletin update(@PathVariable("id") String id, @RequestBody Map<String, Object> updates) {
        // Verificar que el recurso existe antes de actualizar
        Boletin existing = service.findById(id)
            .orElseThrow(() -> new RuntimeException("Boletin not found with id " + id));
    ...
    private void updateEntityFromMap(Boletin entity, Map<String, Object> data) {
