...
import com.example.genapp.service.DocenteService;
...
public class DocenteController {
    ...
    @GetMapping("/{id}")
    public Docente one(@PathVariable("id") String id) {
    return service.findById(id)
    .orElseThrow(() -> new RuntimeException("Docente not found with id " + id));
    }
    ...
    @PutMapping("/{id}")
    public Docente update(@PathVariable("id") String id, @RequestBody Map<String, Object> updates) {
        // Verificar que el recurso existe antes de actualizar
        Docente existing = service.findById(id)
            .orElseThrow(() -> new RuntimeException("Docente not found with id " + id));
    ...
    private void updateEntityFromMap(Docente entity, Map<String, Object> data) {
