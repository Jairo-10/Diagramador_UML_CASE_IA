...
import com.example.genapp.service.CalificacionService;
...
public class CalificacionController {
    ...
    @GetMapping("/{id}")
    public Calificacion one(@PathVariable("id") Long id) {
    return service.findById(id)
    .orElseThrow(() -> new RuntimeException("Calificacion not found with id " + id));
    }

    @PostMapping
    public Calificacion create(@RequestBody Map<String, Object> body) {
    ...
    @PutMapping("/{id}")
    public Calificacion update(@PathVariable("id") Long id, @RequestBody Map<String, Object> updates) {
        // Verificar que el recurso existe antes de actualizar
        Calificacion existing = service.findById(id)
            .orElseThrow(() -> new RuntimeException("Calificacion not found with id " + id));
    ...
    private void updateEntityFromMap(Calificacion entity, Map<String, Object> data) {
        data.forEach((key, value) -> {
            try {
                // Manejar relación ManyToOne: inscripcion
                if (key.equals("inscripcionid") && value != null) {
                    Long relId = convertToLong(value);
                    service.findInscripcionById(relId).ifPresent(entity::setInscripcion);
                    return;
                }
